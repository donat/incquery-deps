...
\code{japc}, \code{japc-ext-cmw} and \code{accsoft-gui-alarms} located in jar
files \code{japc.jar}, \code{japc-ext\-cmw.jar} and
\code{accsoft-gui-alarms.jar} respectively. The first one contains the JAPC core
...
\code{java.*} package). For example, let's see the extracted references
 from the \code{Command\-Panel\-.handle\-Reset()} method defined in the
 \code{japc-ext-cmw} project.
...
\hline
\end{tabl}
The first one is a \code{ConstantMethodRef} type reference. During the analysis
...
finds that there is a \code{setValue()} method defined with the same exact 
...
\code{CommandPanel.handleReset()} method pointing to the
\code{Parameter.setValue()} method.
...
children (as \code{WPackage Fragment} instances). Under the Java packages there
...
explicit queries.
\picr{incdeps.png}{Execute explicit queries from the source code editor}
...
only about his own \code{japc} project. If the \ptool{} is  installed in his
...
Because the \code{japc-ext-cmw} and the \code{accsoft-gui-alarms} projects have
references to the \code{japc} project, both of them are loaded to the repository
model. It has all the element but with the compacted version, so no fully
qualified names. But because there is no overlapping names between the classes
and method than the compacted model does not introduce false-positive results.
...
there was any. Here the \code{cern.japc.Parameter} instance has only the name
\code{Parameter}.
...
The object representing the \code{cern.japc.ParameterFactory\#newParameter()}
method contains only the \code{newParameter} formation and has the reference on
its container.
  \item The dependencies on the figure points to the same elements a it was
  before. For example the method call dependency starts from the object
  representing the \code{setDevice\-Name()}  method
  and shows, that it calls the \code{newParameter()} method from japc's
  \code{ParameterFactory} class.
\end{itemize}
...
The pattern finds, that the \code{newParameter()} method object from the
workspace has a counterpart in the repository: they have the same name, same
container class and they are defined in a project with also a same name.
Then the query finds, that there is a dependency representing a method call
pointing to the \code{newParameter} object in the repository instance model as
it a target of a method call. Because such a dependency has found, the query
return the source part of the dependency object (the dependency's \code{from}
...
the compacted instance model and the workspace instance describing only the
\code{japc} project are loaded.
\picr{modelqueryresult.png}{Development-time evaluation of the pattern}
...
one of them represents a method from a workspace and their dependent counterpart
...
\picr{modelqueryui.png}{Results of the incoming methods query on the
UI}
The displayed query results depend on the selected element on the user
interface. If the selection changes or the workspace model gets modified,
it will automatically trigger an update on the ui components. Through this
the components provides context-dependant and instant feedback.

The second element provides validation over the models. Suppose that somebody
wants to add a new \code{prettyPrint()} method to the japc's \code{Parameter}
...
\picr{queryvalidation.png}{Model query results in error markers on the source
code} The validation marker is a result of a \emph{Commit impact} type
