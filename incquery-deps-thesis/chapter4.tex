...
transforms it into \code{println(java.la} \code{ng.String):void}.
...
\code{java.*} package). For example, let's see the extracted references
 from the \code{CommandPanel .handleReset()} method defined in the
...
\picseventy{cp3model.pdf}{The compacted repository model}
...
Most of the queries are explicitly attached to views to display the results.
This happens in a context-sensitive way: the displayed data depends on the
current selection in the development environment. 

The model queries has a subset called  \emph{validation queries} which are a bit
different from the others. They are searching if the loaded models comply with certain
...
\picr{wsmodel.png}{Workspace model containing the japc project}
...
references to the \code{japc} project, the compacted version of the repository instance model (produced by the
dependency database synchronizer) contains all the three projects from the
repository. Part of the model is shown on
\autoref{fig:cp3modelinstance.pdf}.\picr{cp3modelinstance.pdf}{Instances of the
compacted repository model}
...
there was any. Here the object representing the \code{cern.japc.Parameter} class has only the name
\code{Parameter}.
...
method contains only the \code{newParameter} part and has the reference on
its container.
  ...
  representing the \code{setDevice} \code{Name()} method
  and and targets the \code{newParameter()} method from \code{japc}'s
  \code{Parameter} \code{Factory} class.
\end{itemize}
...
\paragraph{Validation queries}	

Let's examine the validation queries through the following example: Say that
somebody wants to add a new \code{prettyPrint()} method to the \code{japc}'s
\code{Parameter} interface. 
\picr{queryvalidation.png}{Validation query results in error markers on the
source code}

A validation query called \code{incompatibeSourceClassChanges} is defined to
...
The modified \code{Parameter} interfaces matches to this query because of the
\code{prettyPrint()} method. This means if this change was deployed it would
